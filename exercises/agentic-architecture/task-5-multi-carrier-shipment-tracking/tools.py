...
Each check_*_status tool returns whatever raw shape that carrier's own
system would actually return — a different one per carrier. Nothing in
here normalizes them; that's normalize.py's PostToolUse hook's job, applied
by common/agent_loop.py after the dispatcher returns and before the model
...
TOOL_IMPLEMENTATIONS = {
    "check_fedex_status": _check_fedex_status,
    "check_regional_courier_status": _check_regional_courier_status,
    "check_legacy_carrier_status": _check_legacy_carrier_status,
    "issue_shipping_credit": _issue_shipping_credit,
    "escalate_to_claims_desk": _escalate_to_claims_desk,
}
