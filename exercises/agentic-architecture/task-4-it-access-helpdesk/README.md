...
```bash
uv run exercises/agentic-architecture/task-4-it-access-helpdesk/main.py
```
Optionally pass a custom user message as the first argument:
```bash
uv run exercises/agentic-architecture/task-4-it-access-helpdesk/main.py "Hi, I'm Alex Kim (EMP-1). I need access to the shared-drive system, and separately, can you tell me what access I currently have?"
uv run exercises/agentic-architecture/task-4-it-access-helpdesk/main.py "Hi, I'm Jamie Lopez (EMP-2). I need access to Salesforce."
uv run exercises/agentic-architecture/task-4-it-access-helpdesk/main.py "Hi, I'm Alex Kim (EMP-1). I need access to prod-admin."
```
...
- **Programmatic enforcement (hooks, prerequisite gates) vs. prompt-based guidance** — `policy.py`, `main.py`
  ...
  The system prompt also *asks* the model to verify before granting, but `enforce_access_prerequisite` is the actual gate — passed to `run_tool_loop` as `hook=enforce_access_prerequisite` in `main.py`, it runs before every tool call and can block `grant_access` outright, regardless of what the prompt says or whether the model follows it.
