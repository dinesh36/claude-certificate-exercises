...
- **Agentic loop lifecycle (send → inspect stop_reason → execute tools → return results):** [`common/agent_loop.py:47-79`](../../../common/agent_loop.py#L47-L79) — `run_tool_loop` sends the request, checks `response.stop_reason`, dispatches every `tool_use` block, and loops.
- **Loop continues on `"tool_use"`, terminates on `"end_turn"`:** [`common/agent_loop.py:57-58`](../../../common/agent_loop.py#L57-L58) — `if response.stop_reason != "tool_use": break` is the only exit condition; no iteration cap and no parsing of assistant text is used to detect completion.
- **Tool results appended to conversation history:** [`common/agent_loop.py:55`](../../../common/agent_loop.py#L55) (assistant turn) and [`common/agent_loop.py:71-79`](../../../common/agent_loop.py#L71-L79) (tool_result blocks appended as the next user turn) so the model reasons over the updated context on its next call.
