---
name: ship-task
description: Take a built task (or any task-folder change) through the full git/PR lifecycle — branch, commit, PR, merge, and sync master back up. Use when the user asks to open a PR, ship, land, or merge a task.
allowed-tools: Bash, Read, Grep
---

# Ship Task

Runs the git/GitHub side of landing one task's work end to end: branch → commit → PR → merge → resync `master`, with no pause for merge confirmation — the user has authorized this skill to run the full pipeline unattended once invoked. This is the workflow that follows building or updating a task (e.g. with the `new-task` skill) — it does not touch task content itself.

## 1. Identify what's being shipped

- `git status --short` to see every changed/untracked file, not just what's inside the task folder. Ship the whole body of work for this task: its `tasks/<domain-slug>/task-<N>-<slug>/` folder, plus anything else touched to build or land it — `common/` additions or edits, the matching `### Implemented Tasks` row in both `CLAUDE.md` and the root `README.md` (`new-task`'s step 7 adds a row to both; they must ship together), README/skill updates, wiki cross-references, etc. None of that should be left behind uncommitted just because it lives outside the task folder.
- Only exclude a changed file if it's genuinely unrelated to this task (e.g. leftover edits from a different task). If you're not sure whether something belongs, ask the user rather than guessing either way — don't silently drop it and don't silently bundle it in.
- If the remaining changes still span more than one task's worth of work, stop and ask the user how to split them into separate PRs.
- Read `N`, the title, and the domain(s) from that task's README `# Task Statement` header (and/or `CLAUDE.md`'s Tasks section) — needed for the branch name, commit message, and PR text below.

## 2. Pre-flight checks

- Confirm the current branch is `master`, and that `git status`/`git diff` show only the task's shipment as scoped in step 1 (task folder + its supporting files) — no drive-by unrelated changes riding along.
- Run `uv run tasks/<domain-slug>/task-<N>-<slug>/main.py` (and any alternate scenario the task's README documents in "How to run") and confirm it actually executes end to end. Do not proceed to branch/commit/PR on the strength of "should work" — this session's own history has caught real bugs this way.

## 3. Branch creation

- Name: `task-<N>-<kebab-slug>` — copy it straight from the task's own folder name, so the branch and the folder always match.
- `git checkout -b task-<N>-<kebab-slug>` from the current `master`.
- If commits for this task already exist directly on local `master` (this repo's early history was built that way, before this skill existed), stop and ask the user how to proceed instead of silently rewriting `master` — moving existing commits onto a branch requires resetting `master`, which is a destructive operation this skill must never perform unprompted.

## 4. Commit

- Stage everything identified in step 1 as part of this task's shipment — the task folder plus every other file that belongs with it (`common/`, `CLAUDE.md`, README/skill updates, etc.). Pass explicit paths to `git add` (list them out, or `git add -u` plus the specific untracked paths) — never `git add -A`/`git add .`, which would also pick up any unrelated stray changes sitting in the working tree.

<commit_message_template>
  <subject>&lt;Add|Update|Rename|Remove&gt; Task &lt;N&gt;: &lt;Title&gt;</subject>
  ...
  <rule>Match the verb to what actually happened: `Add` for a brand-new task, `Update` for changes to an existing one (README rewrite, scenario swap, bug fix), `Rename` for a folder/number move, `Remove` for deleting one. Check `git log` before picking wording if unsure — this is this repo's established subject style, not a generic convention. Keep the trailer's author name (`Claude Sonnet 5`) matching what `git log` shows; don't substitute a different Co-Authored-By convention.</rule>
</commit_message_template>
...
- [x] `uv run tasks/&lt;domain-slug&gt;/task-&lt;N&gt;-&lt;slug&gt;/main.py "&lt;alt scenario&gt;"` — &lt;what this confirmed, e.g. "narrow scenario, verified it skips the optional fork"&gt;
  </body>
...
- `git push -u origin task-<N>-<kebab-slug>`
...
- Proceed directly to merging once the PR is open — do not wait for the user to say "merge it" first. The gate that actually protects `master` is step 2's pre-flight check (task verified to run) and step 1's scoping (only this task's own files); once those passed, merging is just the mechanical next step, not a separate decision point.
- Default merge strategy: squash, keeping one commit per task in `master`'s history, and delete the branch on merge:
  ```
