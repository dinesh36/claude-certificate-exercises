...
- Boundary violations (e.g. an unrecognized document id, a non-URL passed to a URL-only tool) should `raise ValueError("...")` with a message that explains the boundary AND names the correct tool to use instead — FastMCP catches the exception and surfaces it to the client as a failed tool call automatically (and `logged_tool` logs the error before re-raising it, so it still reaches the client). When the task statement is specifically about *structured* error metadata (errorCategory/isRetryable/description — Task Statement 2.2's shape), raise `common.errors.StructuredToolError(category, is_retryable, description)` instead of a plain `ValueError` — it's a thin wrapper that JSON-encodes a `tool_error()` dict as the exception's message, so the same isError mechanism carries the structured fields through instead of free text.
- Add `mcp[cli]` to the root `pyproject.toml`'s `dependencies` the first time this type is used, if it isn't already there.
- **Most** MCP Server Tasks need no `common/agent_loop.py`, no `common.client`, no `TOOLS` list — `common/mcp_logging.py` is the one shared module this type usually reuses. **Exception:** a task statement about a mechanism that only exists in the raw Anthropic Messages API request (Task Statement 2.3's `tool_choice` is the concrete case — there is no MCP-protocol equivalent of `tool_choice` at all, since it's a parameter the *caller* of the model sets, not something an MCP server's tool definitions can express) needs a hybrid: an MCP tool's *implementation* makes its own internal Anthropic API call(s) via `common/agent_loop.py`'s `run_tool_loop`, exactly like an Agentic Tool-Use task's `tools.py` would (own `_client`/`_model` built at module load time). See `tasks/tool-design-mcp/task-3-tool-distribution-and-choice/pipeline.py` for the reference shape — a `pipeline.py` module holding the internal subagent orchestration, kept separate from `server.py`'s MCP-facing layer. Don't reach for this hybrid by default; only when the task statement's content genuinely has no MCP-side expression, the same way this file itself only exists because Task Statement 2.1/2.2 turned out not to fit the Agentic Tool-Use shape.
- If the task statement is about **distributing tools across agents** specifically (not just tool_choice), also add a nested `.claude/agents/*.md` per role — Claude Code's own subagent feature, whose `tools:` frontmatter can scope a subagent to specific MCP tools (`mcp__<server>__<tool>` or `mcp__<server>__*`) with Claude Code enforcing the restriction itself, rather than only simulating tool-scoping inside an internal pipeline. See `tasks/tool-design-mcp/task-3-tool-distribution-and-choice/.claude/agents/` for the reference shape (one subagent per MCP tool, each scoped to exactly one). These are discovered only when a Claude Code session's working directory is the task folder — same as the MCP registration itself — and can't be exercised via a direct Python call (verify them from an actual session, or note honestly if that isn't possible in the current environment).

## 4. Write README.md

<readme_template>
  <purpose>
    Same purpose as every other type's README: prove, with the real server code pasted in, that every Knowledge-of and Skills-in bullet for the covered task statement is actually implemented — plus prove the server is genuinely usable by documenting the exact terminal commands and prompts a reader needs to register it with Claude Code and exercise it themselves. Follow `SKILL.md`'s "README writing style" section for how to write the prose — short paragraphs, bullets over run-on sentences, simple language.
  </purpose>
