# Preparation Task 2: Configure Claude Code for a Team Development Workflow
...
Source: [`wiki/tasks/6-preparation-tasks.md`](../../../wiki/tasks/6-preparation-tasks.md), Task 2.
...
- **Step 2 — Create `.claude/rules/` files with YAML frontmatter glob patterns for different code areas; verify rules load only when editing matching files** — [`tasks/claude-code-workflows/task-3-path-specific-rules`](../../claude-code-workflows/task-3-path-specific-rules/README.md)
...
- **Step 3 — Create a project-scoped skill in `.claude/skills/` with `context: fork` and `allowed-tools` restrictions; verify it runs in isolation without polluting the main conversation** — [`tasks/claude-code-workflows/task-2-slash-commands-and-skills`](../../claude-code-workflows/task-2-slash-commands-and-skills/README.md)
