# Preparation Task 3: Build a Structured Data Extraction Pipeline
...
Source: [`wiki/tasks/6-preparation-tasks.md`](../../../wiki/tasks/6-preparation-tasks.md), Task 3.
...
- **Step 2 — Implement a validation-retry loop: when validation fails, send a follow-up including the document, the failed extraction, and the specific validation error; track resolvable (format) vs. unresolvable (information absent) errors** — [`tasks/prompt-engineering/task-4-validation-retry-feedback-loops`](../../prompt-engineering/task-4-validation-retry-feedback-loops/README.md)
...
- **Step 3 — Add few-shot examples demonstrating extraction from documents with varied formats (inline citations vs. bibliographies, narrative vs. tables); verify improved handling of structural variety** — [`tasks/prompt-engineering/task-2-few-shot-consistency`](../../prompt-engineering/task-2-few-shot-consistency/README.md)
...
- **Step 4 — Design a batch processing strategy: submit 100 documents via the Message Batches API, handle failures by `custom_id`, resubmit failed documents with modifications, and calculate total processing time relative to SLA constraints** — [`tasks/prompt-engineering/task-5-batch-processing-strategies`](../../prompt-engineering/task-5-batch-processing-strategies/README.md)
...
- **Step 5 — Implement a human review routing strategy: have the model output field-level confidence scores, route low-confidence extractions to human review, and analyze accuracy by document type and field to verify consistent performance** — [`tasks/context-management/task-5-stratified-confidence-calibration`](../../context-management/task-5-stratified-confidence-calibration/README.md)
