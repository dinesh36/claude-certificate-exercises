...
entries. Subagent calls need their own Anthropic client/model, so this module
creates its own (_client/_model below) rather than taking them as parameters
— that binding work stays entirely in this file; main.py just imports TOOLS
and hands it straight to common.agent_loop.run_tool_loop, exactly like every
other task.
"""
...
def _dispatch_blog_review_subagent(post_id: str) -> dict:
    ...
    raw = run_subagent(_client, _model, SUBAGENT_DEFINITIONS["blog_review"]["system_prompt"], user_message)
...
def _dispatch_synthesis_subagent(findings: list[dict], focus: str) -> dict:
    if not findings:
        return tool_error(
            "validation",
            False,
    ...
    summary = run_subagent(_client, _model, SUBAGENT_DEFINITIONS["synthesis"]["system_prompt"], user_message, max_tokens=2048)
...
TOOLS = [
    {
        ...
        "implementation": _dispatch_blog_review_subagent,
    },
    {
        "schema": {
        ...
        "implementation": _dispatch_synthesis_subagent,
    },
    {
        "schema": {
        ...
        "implementation": _dispatch_itinerary_subagent,
    },
]
